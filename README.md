...
- **支付通知**：监听微信 / 支付宝 / 云闪付 / 京东金融 / 美团 / 数字人民币及各大银行 App 的推送通知，自动生成记录
  ...
  - **自动区分交易额与账户余额**：银行 App 通知常见的「…支出 0.50 元，余额 1,234.56 元」只取交易额；几角钱的小额交易同样能正确记录
- **实时刷新**：后台写入记录后通过进程内事件总线通知界面，返回应用或停留在应用内时数据立即更新，无需手动刷新
- **通知诊断**：设置页可查看最近 30 条通知的原文与识别结果（已记账金额 / 未记账的具体原因），便于排查漏记
...
│   │       │   ├── MainActivity.kt
│   │       │   ├── data/         # 数据模型、分类、存储、事件总线、通知日志、账单解析
│   │       │   ├── util/         # 日期、金额工具
│   │       │   ├── ui/           # Compose 界面
│   │       │   │   ├── AppRoot.kt
│   │       │   │   ├── LedgerViewModel.kt
